...
/**
 * CRUD for the "giros del negocio" dropdown list.
 * Same shape as vendedores.
 */
export default async function handler(req, res) {
  try {
    await ensureIndexes()
    const giros = await getCollection('giros')

    if (req.method === 'GET') {
      const incluirInactivos = req.query.incluir_inactivos === 'true'
      const filter = incluirInactivos ? {} : { activo: true }
      const items = await giros.find(filter).sort({ orden: 1, nombre: 1 }).toArray()
    ...
    if (req.method === 'POST') {
      ...
      const last = await giros.find().sort({ orden: -1 }).limit(1).toArray()
      const orden = last.length ? (last[0].orden || 0) + 1 : 1

      try {
        const result = await giros.insertOne({
          nombre,
          activo: true,
          orden,
          created_at: new Date()
        ...
        if (err.code === 11000) return res.status(409).json({ error: 'Giro ya existe' })
        throw err
      }
    }
    ...
    if (req.method === 'PUT') {
      ...
      const result = await giros.findOneAndUpdate(
        { _id: new ObjectId(_id) },
        { $set: update },
        { returnDocument: 'after' }
      )
      if (!result) return res.status(404).json({ error: 'Giro no encontrado' })
    ...
    if (req.method === 'DELETE') {
      ...
      if (!result) return res.status(404).json({ error: 'Giro no encontrado' })
      return res.status(200).json({ ok: true, giro: result })
    }
